...
const LoginScreen = ({ onLogin }) => {
  const [email, setEmail] = React.useState('rohan.kapoor@acmecorp.in');
    ...
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ flex: 1, background: '#FAFAF8', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px' }}>
        <div style={{ width: '100%', maxWidth: 380, animation: 'fadeUp 400ms ease both' }}>
          ...
          {/* Email */}
          <TjInput label="Work Email" placeholder="you@company.com" type="email" value={email} onChange={e => setEmail(e.target.value)} icon="✉" />

          {/* Password */}
          <div style={{ marginBottom: '8px' }}>
            <TjInput label="Password" placeholder="Enter your password" type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} icon="🔒"
          ...
          <div style={{ textAlign: 'right', marginBottom: '22px', marginTop: '-8px' }}>
            <span style={{ fontSize: '12px', color: '#E8783B', cursor: 'pointer', fontWeight: 600 }}>Forgot password?</span>
          </div>
