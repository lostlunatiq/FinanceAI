...
const APClerkDashboard = () => {
  ...
  const [items, setItems] = React.useState([
    { id: 'INV-2024-090', vendor: 'GlobalSync Technologies', amount: '₹1,22,500', date: 'Apr 18', status: 'PENDING_L1', priority: 'high', age: '2 days' },
    { id: 'INV-2024-089', vendor: 'Sigma Electrical Works', amount: '₹2,15,500', date: 'Apr 17', status: 'QUERY_RAISED', priority: 'medium', age: '3 days' },
    { id: 'INV-2024-086', vendor: 'Acme Office Supplies', amount: '₹45,200', date: 'Apr 16', status: 'PENDING_L1', priority: 'low', age: '4 days' },
    { id: 'INV-2024-085', vendor: 'Meridian Logistics', amount: '₹92,300', date: 'Apr 15', status: 'PENDING_L1', priority: 'medium', age: '5 days' },
  ]);

  const confirmAction = (type) => {
    const statusMap = { approve: 'APPROVED', reject: 'REJECTED' };
    setItems(prev => prev.map(i => i.id === modal.inv.id ? { ...i, status: statusMap[type] } : i));
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px', animation: 'fadeUp 250ms 120ms ease both', opacity: 0, animationFillMode: 'forwards' }}>
        <Card style={{ padding: '0', overflow: 'hidden' }}>
          ...
          {items.map((inv, idx) => {
            ...
            return (
              <div key={inv.id} style={{ padding: '16px 22px', borderBottom: '1px solid #F8F7F5', display: 'flex', alignItems: 'center', gap: '16px', opacity: done ? 0.5 : 1, transition: 'opacity 200ms', background: done ? '#F8F7F5' : 'white' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <Card style={{ padding: '20px' }}>
            ...
            {[
              ...
              { task: 'Respond to vendor queries', done: true, count: 1 },
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: i < 3 ? '1px solid #F8F7F5' : 'none' }}>
                <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${t.done ? '#10B981' : '#E2E8F0'}`, background: t.done ? '#10B981' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, cursor: 'pointer' }}>
      ...
      {modal && (
        <TjModal open onClose={() => setModal(null)}
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            ...
            <Btn variant={modal.type === 'approve' ? 'green' : 'destructive'} onClick={() => confirmAction(modal.type)} disabled={modal.type === 'reject' && notes.length < 10}>
...
const FinanceManagerDashboard = () => {
  ...
  const approvalChain = [
    { stage: 'Pending L1', count: 4, amount: '₹4.75L', color: '#F59E0B', items: ['INV-2024-090 · GlobalSync · ₹1,22,500', 'INV-2024-086 · Acme Office · ₹45,200'] },
    { stage: 'Pending HOD', count: 2, amount: '₹3.58L', color: '#E8783B', items: ['INV-2024-089 · Sigma Elec · ₹2,15,500'] },
    { stage: 'Pending Finance Mgr', count: 1, amount: '₹1,22,500', color: '#EF4444', items: ['INV-2024-091 · NovaBridge · ₹8,40,000'] },
        ...
        <Card style={{ padding: '22px' }}>
          ...
          <div style={{ marginTop: '16px', padding: '14px', background: '#FEF3C7', borderRadius: '12px', border: '1px solid #FDE68A' }}>
            ...
            <div style={{ marginTop: '10px' }}>
              <Btn variant="primary" small>Review Now →</Btn>
            </div>
          </div>
        </Card>
            ...
            <div key={i} style={{ padding: '16px 22px', borderBottom: '1px solid #F8F7F5', display: 'flex', alignItems: 'center', gap: '16px', background: p.urgent ? 'rgba(239,68,68,0.02)' : 'white' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '11px', color: '#94A3B8', marginTop: '2px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  {p.bank} · Approved by {p.approved} · Due {p.due}
                </div>
              </div>
...
Object.assign(window, { APClerkDashboard, FinanceManagerDashboard, FinanceAdminDashboard });
