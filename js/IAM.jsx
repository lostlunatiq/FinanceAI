...
const IAMScreen = () => {
  ...
  const [newUser, setNewUser] = React.useState({ username: '', email: '', first_name: '', last_name: '', role: 'employee', password: '' });
  const [inviteError, setInviteError] = React.useState('');
  const [inviteLoading, setInviteLoading] = React.useState(false);

  const { AuthAPI, mapRole } = window.TijoriAPI;
  ...
  // Normalize backend user to display format
  const normalizeUser = (u) => ({
    id: u.id,
    name: `${u.first_name} ${u.last_name}`.trim() || u.username,
    email: u.email || `${u.username}@internal`,
    role: mapRole(u.role, u.is_superuser),
    backendRole: u.role,
  ...
  const filteredUsers = displayUsers.filter(u => {
    const mr = roleFilter === 'All' || u.role === roleFilter || u.backendRole === roleFilter;
  ...
  const handleInviteUser = async () => {
    ...
    try {
      ...
      setInviteOpen(false);
      setNewUser({ username: '', email: '', first_name: '', last_name: '', role: 'employee', password: '' });
      loadUsers();
    } catch (err) {
  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      {tab === 'Users' && (
        <>
          <StatsRow cards={[
            { label: 'Total Users', value: loadingUsers ? '…' : String(displayUsers.length), delta: 'registered', deltaType: 'positive' },
            { label: 'Active', value: loadingUsers ? '…' : String(displayUsers.filter(u => u.status === 'ACTIVE').length), delta: 'Verified', deltaType: 'positive', color: '#10B981' },
            { label: 'Suspended', value: loadingUsers ? '…' : String(displayUsers.filter(u => u.status === 'SUSPENDED').length), delta: 'Access revoked', deltaType: 'negative', color: '#EF4444' },
          ]} />
          ...
          <div style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
            ...
            {loadingUsers && (
              <div style={{ padding: '32px', textAlign: 'center', color: '#94A3B8', fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: '13px' }}>Loading users…</div>
            )}
          </div>
        </>
      )}
      ...
      <TjModal open={inviteOpen} onClose={() => { setInviteOpen(false); setInviteError(''); }} title="Create New User" width={500}>
        ...
        {inviteError && (
          <div style={{ background: '#FEE2E2', border: '1px solid #FECACA', borderRadius: '8px', padding: '10px 14px', marginBottom: '12px', fontSize: '12px', color: '#991B1B', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{inviteError}</div>
        )}
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
          <Btn variant="secondary" onClick={() => { setInviteOpen(false); setInviteError(''); }}>Cancel</Btn>
