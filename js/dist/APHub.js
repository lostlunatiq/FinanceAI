...
const APHubScreen = ({
  ...
  const loadBills = () => {
    ...
    Promise.all([BillsAPI.queue(), DashboardAPI.stats()]).then(([bills, s]) => {
  ...
  const filtered = invoices.filter(inv => {
    const matchF = filter === 'ALL' || (filter === 'ANOMALY' ? inv.anomaly : inv.status.startsWith(filter) || inv.status === filter);
